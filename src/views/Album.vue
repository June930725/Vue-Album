...
export default {
  ...
  methods: {
    onSubmit () {
      ...
      if (this.image.size > 1024 * 1024) {
        ...
        this.axios.post(process.env.VUE_APP_API + '/album/', fd)
          .then(res => {
            if (res.data.success) {
              // 將新增的圖片塞進相簿陣列
              res.data.result.src = process.env.VUE_APP_API + '/album/file/' + res.data.result.file
    ...
    save (image) {
      this.axios.patch(process.env.VUE_APP_API + '/album/' + image._id, { description: image.model })
        .then(res => {
          if (res.data.success) {
            image.edit = false
    ...
    del (image, idx) {
      this.axios.delete(process.env.VUE_APP_API + '/album/' + image._id)
        .then(res => {
          if (res.data.success) {
  ...
  mounted () {
    this.axios.get(process.env.VUE_APP_API + '/album/user/' + this.user.id)
      .then(res => {
        if (res.data.success) {
          this.images = res.data.result.map(image => {
            image.src = process.env.VUE_APP_API + '/album/file/' + image.file
